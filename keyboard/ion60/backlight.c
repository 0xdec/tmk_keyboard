...
static const rgbled_t led_colors[MATRIX_ROWS][MATRIX_COLS];
...
void backlight_send_row(uint8_t row) {
    ...
    for (uint8_t i = 0; i < MATRIX_COLS; i++) {
        ...
        switch (mode) {
            case 0: // Solid color
                TLC.set((i * 3) + 1, led_colors[row][i].red * brightness);
                TLC.set(i * 3, led_colors[row][i].green * brightness);
                TLC.set((i * 3) + 2, led_colors[row][i].blue * brightness);
                break;
            case 1: // Reactive
                if (led_colors[row][i].frame < 64) {
                    ...
                    TLC.set((i * 3) + 1, led_colors[row][i].red * brightness * multiplier / 255);
                    TLC.set(i * 3, led_colors[row][i].green * brightness * multiplier / 255);
                    TLC.set((i * 3) + 2, led_colors[row][i].blue * brightness * multiplier / 255);
...
void backlight_caps_lock(bool state) {
    ...
    led_colors[caps_key->key.row][caps_key->key.column] = temp;
}
